...
import { IconDownOutlined } from 'gupo-icons-vue3';
import { cloneDeep } from 'lodash-unified';
import { computed, nextTick, onMounted, onBeforeUnmount, ref, watch, h, reactive } from 'vue';
import GlobalFormItem from '@src/components/UI/GlobalForm/Item.vue';
import { formItemProps } from '@src/components/UI/GlobalForm/utils';
...
watch(
    ...
    () => {
        ...
        Object.assign(_formData, _defaultFormData);
    },
    { immediate: true }
);
...
<template>
    <div class="global-search" :class="{ active: isLinefeed, sticky: isSticky }">
        ...
        <div class="btn-box" :class="{ isOpen: isOpen, label: isLabelMode }">
            ...
            <span class="operation" @click="isOpen = !isOpen" v-if="isLinefeed">
                {{ isOpen ? '收起' : '展开' }}
                <IconDownOutlined :class="{ active: isOpen }" />
            </span>
        </div>
    </div>
</template>
...
.global-search {
    ...
    &.sticky {
        ...
        top: 0;
        z-index: 10;
        width: calc(100% + @globalSpace * 2);
        padding: calc(@globalSpace - 20px) calc(@globalSpace - 20px) calc(@globalSpace - 9px);
        margin: calc(@globalSpace * -1) calc(@globalSpace * -1) @space5 calc(@globalSpace * -1);
    }
}
</style>
